...
model {
  ...
  // level 2
  for (i in 1:N) {
    p[i] <- x1_coeff[x1[i]] + x2_coeff[x2[i]] + x3_coeff[x3[i]] + x4_coeff[x4[i]]+  x5_coeff[x5[i]]+  x6_coeff[x6[i]];
...
generated quantities{
  ...
  vector[Nt] pt;
  for (i in 1:Nt) {
    pt[i] <- x1_coeff[x1t[i]] + x2_coeff[x2t[i]] + x3_coeff[x3t[i]] + x4_coeff[x4t[i]]+  x5_coeff[x5t[i]]+  x6_coeff[x6t[i]];
